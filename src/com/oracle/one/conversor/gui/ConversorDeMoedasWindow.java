...
import com.oracle.one.conversor.currency.Currency;
import com.oracle.one.conversor.currency.CurrencyConverter;
...
public class ConversorDeMoedasWindow extends JDialog {
  ...
  public ConversorDeMoedasWindow(JFrame owner) {
    ...
    this.valueField.setPreferredSize(
        new Dimension(100, this.valueField.getPreferredSize().height)
      );
    this.inputsPanel.add(this.valueField);

    this.sourceCurrencySelector =
      new CurrencySelector(this, APIHandler.BASE_CURRENCY);
    this.inputsPanel.add(this.sourceCurrencySelector.getButton());

    this.switchBtn = new JButton();

    try {
      this.switchBtn.setIcon(
          new ImageIcon(
            new URL("https://cdn-icons-png.flaticon.com/16/466/466154.png")
          )
        );
    } catch (Exception e) {
    ...
    this.switchBtn.addActionListener(
        new ActionListener() {
          @Override
    ...
    this.convertBtn.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            double value;
            ...
            String msg =
              sourceSymbol + value + " = " + targetSymbol + convertedValue;
            String title =
              "Conversão de " +
              source.getIsoCode() +
              " para " +
